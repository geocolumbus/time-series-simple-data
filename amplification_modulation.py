...
def create_sine_array(size):
    x = np.arange(0, size)
    y = np.sin(x / (365 / np.pi / 32)) * np.sin(x / (365 / np.pi / 2)) + 1
    return y
...
def create():
    print('Creating amp_mod_training.csv')
    save_array_to_csv(create_training_sine_wave("2015-01-01", "2021-05-31"), "csv/amp_mod_training.csv")

    print('Creating amp_mod_prediction.csv')
    save_array_to_csv(create_prediction_sine_wave("2015-01-01", "2021-12-31", 30, "2021-06-01"),
                      "csv/amp_mod_prediction.csv")
