...
def create_sine_array(size):
    x = np.arange(0, size)
    y = np.sin(x / ((365 / 2) / np.pi / 4)) + np.sin(x / ((365 / 2) / np.pi / 12)) + np.sin(
        x / ((365 / 2) / np.pi / 20)) + 3
    return y
...
def create():
    print('Creating sine_training.csv')
    save_array_to_csv(create_training_sine_wave("2015-01-01", "2021-05-31"), "csv/sine_training.csv")

    print('Creating sine_prediction.csv')
    save_array_to_csv(create_prediction_sine_wave("2015-01-01", "2021-12-31", 30, "2021-06-01"),
                      "csv/sine_prediction.csv")
